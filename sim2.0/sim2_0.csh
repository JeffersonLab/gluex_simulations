...
#
cp -pv /group/halld/www/halldweb/html/gluex_simulations/sim2.0/* .
...
#
echo -=-run bggen-=-
cp -v run.ffr.${collimator}_coll.template_${suffix} run.ffr
set seed = $run$file
...
echo -=-run hdgeant-=-
rm -f control.in
cp -v control.in_${collimator}_coll_${suffix} control.in
echo -=-control.in-=- 
...
ls -lt
echo -=-run mcsmear-=-
set run6 = `echo ${RUN} | awk '{printf "%06d\n",$0;}'`
set random_trigger_file = "/cache/halld/RunPeriod-2017-01/sim/random_triggers/run${run6}_random.hddm"
set command = "mcsmear -PJANA:BATCH_MODE=1 -PTHREAD_TIMEOUT=300 -PNTHREADS=1"
set command = "$command hdgeant.hddm ${random_trigger_file}:1"
echo command = $command
$command
